             ...
             int argc,char **argv)
{
    ...
    if (!strncmp(ipath+idx_name,tmpfname,strlen(ipath+idx_name)))
    {
    ...
    if (!strncmp (opath+idx_ext, backup_ext,strlen(opath+idx_ext)))
    {
        printf("'%s':bad filename.\n", opath);
        return (FAILURE);
    }
...
/*-------------------------------------------*/
static unsigned short ihead [0x10];
static WORD ohead [0x10], inf [8];
